...
function handleToggle() {
  if(toggler.checked) {
    localStorage.setItem("isCheked", toggler.checked);
...
async function getContent() {
    ...
    axios.get(`/api/admin/content?sort=-1`)
        .then(function (response) {
            // handle success
            // load data to html
            ...
            let content = "";
              ...
              data.filter(function (item) {
                ...
                if(item.jenis === localStorage.getItem("link_id")) {
...
async function getContentByKategori() {
  ...
  .then(function (response) {
    ...
    <div class="card h-100" style="border: none;">
            <img src="thumbnail/thumbnail1.png" class="card-img-top img-fluid" alt="thumbnail">
            <div class="card-body">
              <h2 class="card-title">Lorem ipsum dolor sit.</h2>
              <small style="color: #C9ADA7;">DISCUSSION COMMUNITY</small><br>
              <small>18-24 Januari 2021</small><br>
              <a class="btn btn-outline-dark" href="#" role="button" style="margin-top: 1rem;border-radius: 24px;width: 10rem;">Daftar Sekarang</a>
            </div>
          </div>`
          if(acara){
            acara.innerHTML = content;
          }
  })
  .catch(function (error) {
      // handle error
...
getContentByKategori();

async function getContentCerpen() {
...
async function putContent() {
  ...
  let src_img = document.getElementById("src_img").value;
  ...
  await axios.put(`/api/admin/content/${id}`, data)
      .then(function (response) {
          // handle success
          window.location.href = `content.html`;
      })
...
async function detailPage() {
    let detailContent = document.getElementById("detail__content");
    let id = localStorage.getItem("id");
    let title = "";
    let srcImg = "";
    let creator = "";
    ...
    axios.get(`/api/admin/content/${id}`)
        .then(function (response) {
            // handle success
            // load data to html
            let data = response.data.data;
            ...
            content += `
            <div class="col-sm-7" id="parent">
          <h1>${data[0].title}</h1>
          <p>
            <small class="text-muted" style="margin-right: 2rem;">Oleh ${data[0].creator}</small>
            <small class="text-muted">${moment(data[0].createdAt).utc(7).format('Do MMMM YYYY')}</small>
          </p>
          <img src=${data[0].src_img} alt="thumbnail" class="img-fluid" width="600px">
          <p>${data[0].content}</p>
            <div class="card-body" style="padding: 2rem;">
              <img src="icon/account.png" alt="profile" class="img-fluid" width="80px">
              <div style="margin-top: -5rem;margin-left: 6rem;">
                <small style="color: rgb(0, 0, 0);">${data[0].creator}</small><br>
                <small style="font-size: 12px;color: rgb(150, 150, 150);">Dosen Fakultas Psikologi UIN Syarif Hidayatullah Jakarta.</small><br>
                <a href="#" style="text-decoration: none;color: black;margin-right: 0.5rem;">
        ...
        <div class="card" style="border-radius: 24px;">
          <div class="card-body" id="card__terbaru">
...
async function getRekomendasi() {
  let rekomendasiFilm = document.getElementById("rekomendasi__film");
  let rekomendaiBuku = document.getElementById("rekomendasi__buku");
  let rekomendasiPodcast = document.getElementById("rekomendasi__podcast");
  await axios.get('api/admin/rekomendasi').then(function (response) {
    let data = response.data.data;
    let rekomendasifilm = "";
    let rekomendasibuku = "";
    let rekomendasipodcast = "";
    data.filter(function (item) {
      if(item.category == "film") {
        ...
        `
        if(rekomendasiFilm) {
          rekomendasiFilm.innerHTML = rekomendasifilm;
        }
      }
      if(item.category == "buku") {
        ...
        `
        if(rekomendaiBuku) {
          rekomendaiBuku.innerHTML = rekomendasibuku;
        }
      }
      if(item.category == "podcast") {
        ...
        if(rekomendasiPodcast) {
          rekomendasiPodcast.innerHTML = rekomendasipodcast;
        }
      }
...
async function getAllContent() {
  ...
  await axios.get('api/admin/content').then(function (response) {
    let data = response.data.data;
    let contentAll = "";
    data.filter(function (item) {
      contentAll += `
